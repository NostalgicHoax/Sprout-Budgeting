...
app.post('/api/move-money', (req, res) => {
  ...
  if (fromCategoryId == null && toCategoryId == null) return bad(res, 'nothing to move');
  if (fromCategoryId === toCategoryId) return bad(res, 'source and destination are the same');
...
function loanFields(body, res) {
  ...
  if (body.loanMonths != null && body.loanMonths !== '') {
    ...
    if (!Number.isInteger(loanMonths) || loanMonths < 1 || loanMonths > 120) return bad(res, 'loan term must be 1-120 months'), null;
  }
...
app.post('/api/accounts', (req, res) => {
  ...
  if (!['cash', 'credit', 'loan'].includes(type)) return bad(res, 'invalid type');
...
app.post('/api/accounts/:id/link', async (req, res) => {
  ...
  if (!conn) return bad(res, 'Choose a connection');
  const externalId = String(req.body?.externalId ?? '');
  if (!externalId) return bad(res, 'Choose a bank account to link');
  ...
  try {
    ...
    return res.status(502).json({ error: `Linked, but bringing in transactions didn't finish: ${e.message}` });
  }
